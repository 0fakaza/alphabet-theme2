...
export type WalletCurrencyPopoverProps = {
  children: React.ReactNode
  hideBalances: boolean
  onToggleHide: () => void
}

export function WalletCurrencyPopover({
  children,
  hideBalances,
  onToggleHide,
}: WalletCurrencyPopoverProps) {
  ...
  return (
    <Popover>
      ...
      <PopoverContent
        align="center"
        sideOffset={15}
        className="z-[55] w-[291px] gap-0 rounded-xl border border-element-border bg-background-elements p-3 shadow-[0px_0px_0px_1px_rgba(0,0,0,0.04),0px_3px_3px_rgba(0,0,0,0.02),0px_6px_6px_rgba(0,0,0,0.04),0px_12px_12px_rgba(0,0,0,0.04),0px_24px_24px_rgba(0,0,0,0.04),0px_48px_48px_rgba(0,0,0,0.04)]"
      >
        {showBonus ? (
          ...
          <div className="flex flex-col gap-2">
            <div className="flex items-center justify-center gap-1 py-2">
              <span className="text-xs font-medium text-text-subtext">Bakiyeleri Gizle</span>
              <button
                type="button"
            ...
            <div className="flex h-12 items-center gap-1.5 rounded-lg border border-element-border bg-background-elements px-4">
              <HugeiconsIcon icon={Search01Icon} className="size-5 shrink-0 text-text-subtext" />
              <input
                type="text"
                placeholder="Ara"
                value={search}
              ...
              {filtered.map((wallet) => (
                ...
                >
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-xs font-medium text-text-main">{wallet.label}</span>
                  </div>
            ...
            <div className="flex flex-col gap-3 px-3 pb-3">
              <span className="text-xs font-medium text-text-subtext">Bahis para birimi</span>
                  ...
                  <label key={code} className="flex cursor-pointer items-center gap-2">
                    ...
                    <span
                      className={`text-sm font-medium tracking-wide ${
                        selectedCurrency === code ? "text-text-main" : "text-text-subtext"
                      }`}
                    >
                      {code}
                    </span>
            ...
            >
              <span className="text-xs font-medium text-text-subtext">Bonus Bakiyelerini gör</span>
